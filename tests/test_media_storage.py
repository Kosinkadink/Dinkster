...
def test_shared_array_view_pins_mapping_after_payload_and_parent_drop(
    ...
    try:
        payload = EncodedPayload.from_buffer(
            ...
            "shm",
            release,
            decode_buffer=decode_audio_buffer if audio else decode_image_array_buffer,
        )
        result = payload.load()
        parent = (
        ...
        assert not parent.flags.owndata
...
def test_buffer_decoder_validates_without_retaining_invalid_mapping(mutation: str) -> None:
...
@pytest.mark.parametrize("audio", [False, True])
def test_boundary_dispatches_zero_copy_decoder_and_releases_derived_view(audio: bool) -> None:
    ...
    try:
        ...
        payload = cast(EncodedPayload, value.payload)
        storage = payload._encoded
        ...
        assert not isinstance(storage, bytes) and storage._materialized is None
        view = array.reshape(-1)[::2]
        del value, result, array, payload
        gc.collect()
        assert storage._borrows == 1
        np.testing.assert_array_equal(view, samples.reshape(-1)[::2])
        del view
        gc.collect()
        assert storage._borrows == 0
        del storage
        gc.collect()
    finally:
...
def test_storage_conversion_and_borrow_preserve_media_semantics(dtype: str) -> None:
    ...
    borrowed = payload.load()
    converted = image_input(borrowed)
    assert media_semantics(borrowed) == media_semantics(image)
    assert media_semantics(converted) == media_semantics(image)
    assert encode_image_array(borrowed) == encoded
    assert image_array_meta(converted)["dtype"] == "float32"
    view = np.asarray(borrowed)[0]
    del converted, borrowed, payload
    gc.collect()
    assert not releases
    np.testing.assert_array_equal(view, image[0])
    del view
    gc.collect()
