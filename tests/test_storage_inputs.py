...
def test_storage_schema_roundtrip_elaboration_and_cache_identity() -> None:
    ...
    for version in SCHEMA_WIRE_SERVE_VERSIONS:
        wire = schema_to_wire(ordinary, wire_version=version)
        assert "acceptsStorage" not in cast(list[dict[str, Any]], wire["interface"])[0]
        assert schema_from_wire(wire) == ordinary
        if version < 39:
            with pytest.raises(ValueError, match="acceptsStorage"):
                schema_to_wire(compact, wire_version=version)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("dinkster_nodes_partner", "register_partner_types"),
        ("dinkster.comfy_compose", "register_comfy_host_types"),
        *[
            (f"dinkster_vision_{name}.nodes", "register_types")
            for name in (
                "birefnet",
                "depth_anything_v2",
                "depth_anything_v3",
...
def test_production_media_registrations_attach_input_hooks(module: str, registration: str) -> None:
    ...
    for spec in media:
        ...
        assert spec.decode_buffer is not None
        if spec.meta is not audio_meta:
            assert spec.decode_buffer is decode_image_array_buffer


def test_buffer_decoder_requires_a_declared_codec() -> None:
    types = TypeRegistry()
    assert types.register("unknown").decode_buffer is None
    with pytest.raises(ValueError, match="buffer decoding requires a declared codec"):
        types.register("invalid", decode_buffer=decode_image_array_buffer)


@pytest.mark.parametrize("encoded", [False, True])
@pytest.mark.parametrize("kind", ["uint8", "uint16", "float16", "bf16", "float32", "audio"])
@pytest.mark.parametrize(
    "runtime", ["numpy", "torch"] if importlib.util.find_spec("torch") else ["numpy"]
)
def test_equivalent_media_keeps_buffer_decoder_and_borrowed_views(
    encoded: bool, kind: str, runtime: str
) -> None:
    ...
    media = TypeRegistry()
    type_id = "comfy.AUDIO" if kind == "audio" else "comfy.IMAGE"
    if runtime == "torch":
        (register_audio_type if kind == "audio" else register_image_type)(media, type_id)
    else:
        register_comfy_host_types(media)
    spec = media.spec(type_id)
    decode_buffer = spec.decode_buffer
    assert decode_buffer is not None
    released: list[str] = []

    def buffer_decoder(data: memoryview, release: Callable[[], None]) -> object:
        assert data.readonly

        def release_borrow() -> None:
            released.append("borrow")
            release()

        return decode_buffer(data, release_borrow)

    def bytes_decoder(data: bytes) -> object:
        raise AssertionError("equivalent media must use its buffer decoder")

    types = TypeRegistry()
    types.register("source", encode=spec.encode, decode=spec.decode)
    types.register("target", encode=spec.encode, decode=bytes_decoder, decode_buffer=buffer_decoder)
    types.register_type_equivalence("source", "target", provider_id="test.media-equivalence")
    obj = (
        {"waveform": np.array([[[-32768, 0, 32767]]], dtype=np.int16), "sample_rate": 48000}
        if kind == "audio"
        else np.asfortranarray(np.tile(pixels(kind), (1, 2, 1, 1)))
    )
    value = types.wrap("source", obj)
    if encoded:
        value = replace(
            value,
            payload=EncodedPayload.from_buffer(
                "source",
                memoryview(bytearray(spec.encode(obj))),
                spec.decode,
                "shm",
                lambda: released.append("storage"),
            ),
        )
    bridged = types.bridge_equivalent(value, "target")
    assert isinstance(bridged.payload, EncodedPayload)
    assert bridged.payload.decode_buffer is buffer_decoder
    decoded = cast(Any, bridged.resolve())
    array = decoded["source"]["pcm"] if kind == "audio" else decoded
    with bridged.payload.borrow_data() as view:
        backing = np.frombuffer(view, dtype=np.uint8)
        pointer = array.data_ptr() if runtime == "torch" else array.ctypes.data
        assert backing.ctypes.data <= pointer < backing.ctypes.data + backing.nbytes
        del backing
    if runtime == "numpy":
        assert not array.flags.writeable
        assert array.dtype == (np.int16 if kind == "audio" else pixels(kind).dtype)
    else:
        dtype = {"bf16": "bfloat16", "audio": "int16"}.get(kind, kind)
        assert str(array.dtype) == f"torch.{dtype}"
    child = array[..., :1]
    del decoded, array, bridged, value
    gc.collect()
    assert released == []
    assert child.shape[-1] == 1
    del child
    gc.collect()
    assert released == (["borrow", "storage"] if encoded else ["borrow"])
