...
from dinkster_compat_comfy.native_arm import GENERATION_PROVIDER_NODES, NATIVE_ARM_NODES
...
def test_compat_provider_executes_every_generation_schema_exactly() -> None:
    ...
    native_arm_node_ids = {node.schema().node_type for node in NATIVE_ARM_NODES}
    assert set(GENERATION_NODE_IDS).intersection(dict(manifest.arms)["native"]) == set(
        GENERATION_NODE_IDS
    ).intersection(native_arm_node_ids)
