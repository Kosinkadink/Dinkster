"""Torch-free Krea 2 split-component planning and identity tests."""
...
from dinkster_inference import krea2_component as component
...
def test_split_component_binds_asset_identity_and_compute_dtype(
    ...
    planned = component.plan_krea2_split_component(
        cast("WeightSource", _Source(path, 10, digest)), role="qwen3vl_4b", path=path
    )
    identity = component.krea2_component_runtime_identity(planned, "qwen3vl_4b", BFLOAT16)

    assert f"asset_digest={digest}" in planned.identity_facts
    assert "asset_size=10" in planned.identity_facts
    assert identity != component.krea2_component_runtime_identity(planned, "qwen3vl_4b", FLOAT32)
    ComponentBinding("qwen3vl_4b", "dinkster.krea2", identity)
...
def test_same_geometry_different_bytes_never_share_an_identity(
    ...
    first = component.plan_krea2_split_component(
        cast("WeightSource", _Source(path, 10, "blake3:" + "1" * 64)),
        role="diffusion",
        path=path,
    )
    second = component.plan_krea2_split_component(
        cast("WeightSource", _Source(path, 10, "blake3:" + "2" * 64)),
        role="diffusion",
        path=path,
    )

    assert component.krea2_component_runtime_identity(
        first, "diffusion", BFLOAT16
    ) != component.krea2_component_runtime_identity(second, "diffusion", BFLOAT16)
...
def test_split_component_refuses_path_mismatch_and_missing_asset_identity(
    ...
    with pytest.raises(component.Krea2ComponentAssemblyError, match="path differs"):
        component.plan_krea2_split_component(
            cast("WeightSource", _Source(path, 5, "blake3:" + "3" * 64)),
            role="diffusion",
            path=tmp_path / "other.safetensors",
        )
    with pytest.raises(component.Krea2ComponentAssemblyError, match="must carry asset identity"):
        component.plan_krea2_split_component(
...
def test_split_component_wraps_geometry_refusal(
    ...
    def refuse(_source: object, _role: object) -> ComponentPlan[object]:
        raise ValueError("not the exact Krea 2 layout")

    monkeypatch.setattr(component, "plan_krea2_component", refuse)

    with pytest.raises(component.Krea2ComponentAssemblyError, match="exact Krea 2 layout"):
        component.plan_krea2_split_component(
...
def test_component_identity_requires_the_matching_role(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="requires the qwen3vl_4b plan"):
        component.krea2_component_runtime_identity(planned, "qwen3vl_4b", BFLOAT16)
