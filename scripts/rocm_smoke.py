"""Report what torch and Dinkster's torch layer can do on an AMD ROCm GPU.

Run under the isolated ROCm env (scripts/setup_env_rocm.ps1 or .sh):
...
ROCm torch builds expose AMD GPUs through the CUDA namespace, so the
backend identity is torch.version.hip, never the device type string.
...
checks: host identity, driver, torch/HIP versions, device name and gfx
architecture, fp32/fp16/bf16 storage and matmul, fp16 SDPA, memory APIs,
cache control, attention route evidence, and GGUF/INT8/FP8 dequantization
parity against the CPU baseline. Exits nonzero when torch is not a HIP
build, no device is visible, a required compute, memory, routing, policy,
or dequantization probe fails, or the report is incomplete; unsupported
quant storage and cast probes are reported, not fatal - refusing them
cleanly is the expected state.
"""
...
def _driver_identity() -> str:
    """Best available display-driver identity for the evidence report."""
    if sys.platform == "win32":
        ...
        try:
            ...
            pass
        return "unknown (Win32_VideoController query failed)"
    parts: list[str] = []
    ...
    if not parts and Path("/sys/module/amdgpu").is_dir():
        # The in-tree amdgpu module has no version file; for in-tree builds
        # the kernel release is the driver version.
        parts.append(f"amdgpu in-tree kernel driver, kernel {platform.uname().release}")
    if parts:
        return "; ".join(parts)
    return "unknown (amdgpu module not loaded and no /opt/rocm/.info/version)"
...
def main() -> int:
    ...
    print(f"hip:      {hip}")
    if hip is None:
        print("error: this torch is not a HIP (ROCm) build", file=sys.stderr)
        return 1
    if not torch.cuda.is_available():
        print("error: torch cannot see a ROCm device on this machine", file=sys.stderr)
        return 1
    device = torch.device("cuda:0")

    driver = _driver_identity()
    print(f"driver:   {driver}")

    devices: list[dict[str, object]] = []
    for index in range(torch.cuda.device_count()):
        properties = torch.cuda.get_device_properties(index)
        # No fallback: a HIP device without a gfx architecture name is an
    ...
    def read_memory() -> None:
        ...
        memory["allocator_allocated"] = int(statistics.get("allocated_bytes.all.current", 0))
        memory["allocator_reserved"] = int(statistics.get("reserved_bytes.all.current", 0))

    record("mem_get_info", read_memory)
    record("empty_cache", torch.cuda.empty_cache)
    record("synchronize", lambda: torch.cuda.synchronize(device))
    if memory:
        gib = 1024**3
        print(
    ...
    report: dict[str, object] = {
        ...
        "driver": driver,
        "torch": {"version": str(torch.__version__), "backend_runtime": f"hip {hip}"},
        "devices": devices,
        "memory": memory,
    ...
    report_problems = validate_smoke_report(report, accelerator="rocm")
    gate_problems = smoke_gate_problems(report, accelerator="rocm")
    ...
    print(format_smoke_receipt(report, accelerator="rocm"))
    if gate_problems:
        for problem in gate_problems:
            if problem not in report_problems:
                print(f"error: smoke gate: {problem}", file=sys.stderr)
        return 1
    print("baseline float compute on ROCm: ok")
    return 0
