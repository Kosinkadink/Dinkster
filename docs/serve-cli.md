...
schemas and the `dinkster-native` provider. Set `DINKSTER_COMFYUI_PYTHON` to the
native execution interpreter when it differs from the host. Use the same
`--remote-catalog-base` and `--remote-gateway-base` options as the server, or
their environment variables, when preparing remote nodes. Direct pack authors
can use `dinkster-doctor path/to/dinkster-pack.toml`. Import/schema failures invalidate the
