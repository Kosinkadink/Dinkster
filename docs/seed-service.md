...
Upload rate applies to both LAN and internet; zero means unlimited. General
installs keep P2P downloads and seeding off by default; explicitly starting
this seed-only service enables global downloading and continuous global seeding.
The existing metered-network pause policy still applies. `disable` is one
...
path and the storage owner's platform fingerprint. Unchanged mappings reuse
