"""Universal TripoSplat generation schemas."""
...
class _TripoSplatSchema(Node):
    @classmethod
    def _requires_provider(cls) -> Mapping[str, object]:
        raise RuntimeError(f"{cls.schema().node_type} requires an execution provider")


class LoadTripoSplatVisionEncoderSchema(_TripoSplatSchema):
    @classmethod
    ...
    def execute(cls, vision_encoder: object) -> Mapping[str, object]:
        del vision_encoder
        return cls._requires_provider()


class LoadTripoSplatDecoderSchema(_TripoSplatSchema):
    @classmethod
    def define_schema(cls) -> NodeSchema:
        return NodeSchema(
...
class TripoSplatPreprocessImageSchema(_TripoSplatSchema):
    @classmethod
    def define_schema(cls) -> NodeSchema:
        return NodeSchema(
    ...
    def execute(
        ...
        del image, mask, erode_radius, size
        return cls._requires_provider()


class TripoSplatConditioningSchema(_TripoSplatSchema):
    @classmethod
    def define_schema(cls) -> NodeSchema:
        return NodeSchema(
    ...
    def execute(cls, vision: object, vae: object, image: object) -> Mapping[str, object]:
        del vision, vae, image
        return cls._requires_provider()


class TripoSplatDecodeSchema(_TripoSplatSchema):
    @classmethod
    def define_schema(cls) -> NodeSchema:
        return NodeSchema(
    ...
    def execute(
        ...
        del samples, decoder, num_gaussians, seed
        return cls._requires_provider()


TRIPOSPLAT_SCHEMA_NODES: tuple[type[Node], ...] = (
...
TRIPOSPLAT_SCHEMA_NODE_IDS = tuple(node.schema().node_type for node in TRIPOSPLAT_SCHEMA_NODES)
