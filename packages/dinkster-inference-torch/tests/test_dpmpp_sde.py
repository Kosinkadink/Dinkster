"""Exact pinned-source trajectory proofs for torch-native DPM++ SDE."""
...
GOLDEN = load_platform_golden(Path(__file__).parent / "goldens/dpmpp_sde_trajectory_goldens.json")
...
@pytest.mark.parametrize(
    ("case_name", "descriptor_id", "parameterization"),
    [
        ("eps", "dinkster.dpmpp_sde", Parameterization.EPS),
        ("flow", "dinkster.dpmpp_sde_gpu", Parameterization.FLOW),
        ("flow", "dinkster.dpmpp_sde", Parameterization.IMAGE_TO_IMAGE_FLOW),
    ],
)
def test_torch_dpmpp_sde_matches_every_executed_reference_seam(
    case_name: str,
    descriptor_id: str,
    parameterization: Parameterization,
) -> None:
    case: dict[str, Any] = GOLDEN["cases"][case_name]
    descriptor = torch_sampler_registry().get(descriptor_id)
...
def test_dpmpp_sde_golden_comes_from_the_pinned_reference() -> None:
