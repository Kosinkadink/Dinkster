...
from dinkster_inference_torch import krea2_component as component
from dinkster_inference_torch.krea2_dit import Krea2DiT
...
def _patch_planning(
    ...
    *,
    ...
    monkeypatch.setattr(component, "plan_krea2_split_component", plan)
    monkeypatch.setattr(component, "krea2_component_runtime_identity", identity)
    return planned
...
@pytest.mark.parametrize(
    ("role", "builder"),
    (
        ("diffusion", component.Krea2DiT),
        ("qwen3vl_4b", component._build_krea2_text),  # pyright: ignore[reportPrivateUsage]
    ),
)
def test_component_loader_preserves_identity_and_open_descriptor(
    ...
    asset = _asset(path)
    expected_identity = "native:dinkster.krea2:" + "a" * 64
    seen: dict[str, object] = {}
    ...
    loaded = component.load_krea2_component(
        path,
        asset=asset,
...
def test_component_loader_refuses_identity_mismatch_before_payload_load(
    ...
    _patch_planning(
        monkeypatch,
        expected_identity="native:dinkster.krea2:" + "b" * 64,
        seen=seen,
    )
    ...
    with pytest.raises(Krea2ComponentAssemblyError, match="expected component identity"):
        component.load_krea2_component(
            path,
            asset=_asset(path),
            expected_role="diffusion",
            expected_identity="native:dinkster.krea2:" + "c" * 64,
            compute_dtype=torch.bfloat16,
        )
...
def test_component_loader_refuses_size_unavailable_and_invalid_arguments(
    ...
    path = _component_path(tmp_path)
    asset = _asset(path)
    identity = "native:dinkster.krea2:" + "d" * 64
    mismatched = AssetRef(asset.digest, asset.name, asset.size + 1, resolver=_FixedResolver(path))
    with pytest.raises(Krea2ComponentAssemblyError, match="byte size differs"):
        component.load_krea2_component(
            path,
            asset=mismatched,
            expected_role="diffusion",
    ...
    with pytest.raises(Krea2ComponentAssemblyError, match="artifact is unavailable"):
        component.load_krea2_component(
            path,
            asset=unavailable,
            expected_role="diffusion",
...
def test_text_builder_refuses_non_exact_profiles() -> None:
    with pytest.raises(Krea2ComponentAssemblyError, match="exact supported profile"):
        component._build_krea2_text(  # pyright: ignore[reportPrivateUsage]
            SimpleNamespace(), operations=cast("Any", None)
        )


class _HeaderSource:
    """Geometry-only WeightSource carrying the exact planner header."""

    def __init__(self, path: Path, layout: Mapping[str, tuple[int, ...]]) -> None:
        self.path = path
        self._geometries = {key: TensorGeometry(shape, BFLOAT16) for key, shape in layout.items()}
...
def _build_text_module(config: object) -> torch.nn.Module:
    return component._build_krea2_text(  # pyright: ignore[reportPrivateUsage]
        config, operations=INITLESS
    )


def _build_dit_module(config: object) -> torch.nn.Module:
    return Krea2DiT(cast("Any", config), operations=INITLESS)


@pytest.mark.parametrize(
    ("role", "layout", "build"),
    [
        ("qwen3vl_4b", krea2_text_layout(), _build_text_module),
        ("diffusion", krea2_layout(), _build_dit_module),
    ],
)
def test_plan_model_keys_match_the_loaded_module_exactly(
    ...
    """Every planned model key must name a module parameter and cover
    all of them: strict load_state_dict rejects any drift (the seam
    the component loader's monkeypatched tests cannot see)."""
    plan = plan_krea2_component(
        cast("Any", _HeaderSource(Path("/fake/component.safetensors"), layout)),
        cast("Any", role),
    )
