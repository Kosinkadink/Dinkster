...
def parse_container(data: bytes) -> tuple[JsonObject, bytes | None]:
    if data[:4] == b"glTF":
        if len(data) < 12:
        ...
        if total_length != len(data):
            raise ValueError(
                f"GLB header declares {total_length} bytes but file contains {len(data)}"
            )
        json_chunk = None
        bin_chunk = None
        offset = 12
        while offset < total_length:
            if offset + 8 > total_length:
                raise ValueError("GLB file truncated in chunk header")
            chunk_len, chunk_type = struct.unpack_from("<II", data, offset)
            offset += 8
            chunk_end = offset + chunk_len
            if chunk_end > total_length:
                raise ValueError("GLB file truncated in chunk payload")
...
def _resolve_uri(uri: str, base_dir: str | None) -> bytes:
    ...
    if scheme:
        raise ValueError(
            f"glTF references URI scheme {scheme!r}; only relative file paths are allowed"
        )
    if base_dir is None:
        raise ValueError(
...
def load_buffers(gltf: JsonObject, bin_chunk: bytes | None, base_dir: str | None) -> list[bytes]:
    buffers = []
    for index, buf in enumerate(gltf.get("buffers", [])):
        if "uri" in buf:
            data = _resolve_uri(buf["uri"], base_dir)
        else:
        ...
        if not isinstance(declared, int) or declared < 0:
            raise ValueError(f"glTF buffer {index} has invalid byteLength")
        if len(data) < declared:
            raise ValueError(
                f"glTF buffer {index} declares {declared} bytes but only {len(data)} are available"
            )
        buffers.append(data)
    return buffers
...
def _view_bounds(
    gltf: JsonObject, buffers: list[bytes], view_index: int
) -> tuple[bytes, int, int]:
    ...
    if not isinstance(start, int) or start < 0 or not isinstance(length, int) or length < 0:
        raise ValueError(f"glTF bufferView {view_index} has invalid byte bounds")
    if start + length > len(buf):
        raise ValueError(f"glTF bufferView {view_index} extends beyond its buffer")
    return buf, start, length


def _view_data(gltf: JsonObject, buffers: list[bytes], view_index: int) -> bytes:
    buf, start, length = _view_bounds(gltf, buffers, view_index)
    return buf[start : start + length]


def read_accessor(gltf: JsonObject, buffers: list[bytes], index: int) -> tuple[np.ndarray, bool]:
    acc = gltf["accessors"][index]
    count = acc["count"]
    if not isinstance(count, int) or count < 0:
        raise ValueError(f"glTF accessor {index} has invalid count")
    ...
    if "bufferView" in acc:
        ...
        buf, view_start, view_length = _view_bounds(gltf, buffers, acc["bufferView"])
        accessor_offset = acc.get("byteOffset", 0)
        if not isinstance(accessor_offset, int) or accessor_offset < 0:
            raise ValueError(f"glTF accessor {index} has invalid byteOffset")
        stride = view.get("byteStride") or elem
        if not isinstance(stride, int) or stride < elem or stride % dtype.itemsize:
            raise ValueError(f"glTF accessor {index} has invalid byteStride")
        required = accessor_offset + ((count - 1) * stride + elem if count else 0)
        if required > view_length:
            raise ValueError(f"glTF accessor {index} extends beyond its bufferView")
        start = view_start + accessor_offset
        if stride == elem:
            arr = np.frombuffer(buf, dtype, count * ncomp, start).reshape(count, ncomp).copy()
        else:
            if count:
                raw = np.frombuffer(buf, np.uint8, (count - 1) * stride + elem, start)
                rows = np.lib.stride_tricks.as_strided(raw, (count, elem), (stride, 1))
                arr = np.ascontiguousarray(rows).view(dtype).reshape(count, ncomp)
            else:
                arr = np.empty((0, ncomp), dtype)
    else:
    ...
    if sparse:
        n = sparse["count"]
        if not isinstance(n, int) or n < 0 or n > count:
            raise ValueError(f"glTF accessor {index} has invalid sparse count")
        idx_def = sparse["indices"]
        val_def = sparse["values"]
        if idx_def["componentType"] not in (5121, 5123, 5125):
            raise ValueError(f"glTF accessor {index} has invalid sparse index component type")
        idx_dtype = np.dtype(_COMPONENT_DTYPES[idx_def["componentType"]])
        ibuf, istart, ilength = _view_bounds(gltf, buffers, idx_def["bufferView"])
        ioffset = idx_def.get("byteOffset", 0)
        indices_end = ioffset + n * idx_dtype.itemsize if isinstance(ioffset, int) else -1
        if not isinstance(ioffset, int) or ioffset < 0 or indices_end > ilength:
            raise ValueError(f"glTF accessor {index} sparse indices exceed their bufferView")
        sidx = np.frombuffer(
            ibuf, idx_dtype, n, istart + ioffset
        ).astype(np.int64)
        vbuf, vstart, vlength = _view_bounds(gltf, buffers, val_def["bufferView"])
        voffset = val_def.get("byteOffset", 0)
        if not isinstance(voffset, int) or voffset < 0 or voffset + n * elem > vlength:
            raise ValueError(f"glTF accessor {index} sparse values exceed their bufferView")
        svals = np.frombuffer(
            vbuf, dtype, n * ncomp, vstart + voffset
        ).reshape(n, ncomp)
        if n and (sidx.max() >= count or len(np.unique(sidx)) != n):
            raise ValueError(f"glTF accessor {index} has invalid sparse indices")
        arr[sidx] = svals
...
def _node_local_matrix(node: JsonObject) -> np.ndarray:
    if "matrix" in node:
...
def _instance_matrices(
    gltf: JsonObject, buffers: list[bytes], node: JsonObject, warn: Warn
    ...
    if len(counts) > 1:
        raise ValueError(
            "EXT_mesh_gpu_instancing attribute accessors disagree on instance "
            f"count: {sorted(counts)}"
        )
    if not any(k in attrs for k in ("TRANSLATION", "ROTATION", "SCALE")):
        warn(
            "instancing-custom",
            "EXT_mesh_gpu_instancing has only custom attributes; importing a single copy",
        )
        return None
    translation = (
        to_float(*read_accessor(gltf, buffers, attrs["TRANSLATION"]))
        if "TRANSLATION" in attrs
        else None
    )
    rotation = (
        to_float(*read_accessor(gltf, buffers, attrs["ROTATION"])) if "ROTATION" in attrs else None
    )
...
def _iter_mesh_nodes(
    gltf: JsonObject, buffers: list[bytes], warn: Warn
) -> Iterator[tuple[JsonObject, np.ndarray]]:
    ...
    while stack:
        ...
        if "mesh" in node:
            ...
            if instances is None:
                ...
                warn(
                    "instancing",
                    "EXT_mesh_gpu_instancing: expanding "
                    f"{len(instances)} instances into merged geometry",
                )
...
def _vertex_attr(
    ...
    if arr.shape[0] < n_verts:
        warn(
            f"count:{label}",
            f"{label} has {arr.shape[0]} entries for {n_verts} vertices; attribute dropped",
        )
        return None
    return arr[:n_verts]


def load_scene_geometry(gltf: JsonObject, buffers: list[bytes], warn: Warn) -> list[MeshPrimitive]:
    required = set(gltf.get("extensionsRequired", []))
    unsupported = required - _SUPPORTED_REQUIRED
    if unsupported:
        raise ValueError(
            f"glTF requires extensions this loader does not support: {sorted(unsupported)}"
        )
    ...
    for node, world in _iter_mesh_nodes(gltf, buffers, warn):
        ...
        for prim in gltf["meshes"][node["mesh"]].get("primitives", []):
            ...
            if "indices" in prim:
                indices = (
                    read_accessor(gltf, buffers, prim["indices"])[0].reshape(-1).astype(np.int64)
                )
            else:
            ...
            if "TANGENT" in attrs:
                ...
                if tan is not None and tan.shape[1] == 4:
                    ...
                    out["tangents"] = np.ascontiguousarray(
                        np.concatenate([txyz, tw], axis=1), np.float32
                    )
            prims.append(out)
    return prims
...
def _decode_texture(
    ...
    if tex_info.get("texCoord", 0) != 0:
        warn(
            f"texcoord:{label}",
            f"{label} uses TEXCOORD_{tex_info['texCoord']}; MESH only carries TEXCOORD_0",
        )
    ...
    if source is None:
        warn(
            f"compressed:{label}",
            f"{label}: compressed texture (basisu/ktx2) without fallback; skipped",
        )
        return None
...
def extract_material(
    ...
    result["texture"] = _decode_texture(
        gltf, buffers, base_dir, pbr.get("baseColorTexture"), warn, "baseColorTexture"
    )
    mr_info = pbr.get("metallicRoughnessTexture")
    result["metallic_roughness"] = _decode_texture(
        gltf, buffers, base_dir, mr_info, warn, "metallicRoughnessTexture"
    )
    normal_info = mat.get("normalTexture")
    result["normal_map"] = _decode_texture(
        gltf, buffers, base_dir, normal_info, warn, "normalTexture"
    )
    result["emissive"] = _decode_texture(
        gltf, buffers, base_dir, mat.get("emissiveTexture"), warn, "emissiveTexture"
    )
    ...
    if occlusion is not None:
        if mr_info is not None and occlusion["index"] == mr_info["index"]:
            ...
            warn(
                "occlusion",
                "standalone occlusionTexture not representable in MESH (ORM packing only); skipped",
            )
...
def load_gltf(
    data: bytes, base_dir: str | None, warn: Warn
) -> tuple[JsonObject, list[bytes], list[MeshPrimitive]]:
    gltf, bin_chunk = parse_container(data)
    version = str(gltf.get("asset", {}).get("version", ""))
    if version.partition(".")[0] != "2":
        raise ValueError(
            f"unsupported glTF asset version {version or 'unknown'}; only glTF 2.x is supported"
        )
