...
def load_obj(data: bytes) -> MeshPrimitive:
    text = data.decode("utf-8", errors="replace")
    text = text.replace("\r\n", "\n").replace("\\\n", "")
    positions: list[tuple[float, ...]] = []
    pos_colors: list[tuple[float, ...]] = []
    uvs: list[tuple[float, float]] = []
    normals: list[tuple[float, ...]] = []

    corner_map: dict[tuple[int, int | None, int | None], int] = {}
    out_pos: list[tuple[float, ...]] = []
    out_col: list[tuple[float, ...]] = []
    out_uv: list[tuple[float, float]] = []
    out_nrm: list[tuple[float, ...]] = []
    ...
    prim: MeshPrimitive = {
        ...
        "colors": _srgb_to_linear(np.clip(np.array(out_col, np.float32), 0.0, 1.0))
        if has_color
        else None,
        "normals": None,
        "tangents": None,
...
def load_stl(data: bytes) -> MeshPrimitive:
...
def _load_stl_binary(data: bytes, n_faces: int) -> MeshPrimitive:
...
def _stl_binary_colors(header: bytes, attr: np.ndarray) -> np.ndarray | None:
    ...
    per_face = (
        np.stack([a & 0x1F, (a >> 5) & 0x1F, (a >> 10) & 0x1F], axis=1).astype(np.float32) / 31.0
    )
...
def _stl_prim(
    positions: np.ndarray, normals: np.ndarray, colors: np.ndarray | None = None
) -> MeshPrimitive:
    if positions.shape[0] == 0:
        raise ValueError("STL contains no facets")
    faces = np.arange(positions.shape[0], dtype=np.int64).reshape(-1, 3)
    lengths = np.linalg.norm(normals, axis=1, keepdims=True)
    normalized = normals / np.maximum(lengths, 1e-12) if float(lengths.max()) > 1e-6 else None
    return {
