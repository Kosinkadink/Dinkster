...
_LINALG_ERROR: type[RuntimeError] = vars(torch.linalg)["LinAlgError"]


class _CentroidTree(Protocol):
    def query(
        self, points: np.ndarray, *, k: int, workers: int
...
def _build_centroid_tree(tri_verts: torch.Tensor) -> _CentroidTree:
    ...
    tree_type = vars(scipy.spatial)["cKDTree"]
    return cast(
        _CentroidTree,
        tree_type(
            tri_verts.mean(dim=1).detach().cpu().numpy(), balanced_tree=False, compact_nodes=False
        ),
    )
...
def _build_tri_spatial_hash(
    centroids: torch.Tensor, tri_radii: torch.Tensor, cell_size: torch.Tensor
):
    ...
    cum = torch.cat(
        [torch.zeros(1, device=device, dtype=n_per_tri.dtype), n_per_tri.cumsum(0)[:-1]]
    )
    ...
    cell_starts = torch.cat([torch.zeros(1, dtype=counts.dtype, device=device), counts.cumsum(0)])
...
def _udf_query(
    ...
    out_tri = torch.full((Q,), -1, dtype=torch.long, device=device) if return_tri_idx else None

    rng = torch.tensor([-1, 0, 1], device=device, dtype=torch.long)
    offs = torch.stack(torch.meshgrid(rng, rng, rng, indexing="ij"), dim=-1).reshape(
        -1, 3
    )  # (27, 3)
    ...
    for cs in range(0, Q, chunk):
        ...
        slot_within = torch.arange(total, device=device) - torch.repeat_interleave(
            slot_starts_per_pair, c_counts
        )
        ...
        local_min = torch.full(
            (qp.shape[0],), float(max_dist) ** 2, dtype=query_points.dtype, device=device
        )
        ...
        if return_closest or return_tri_idx:
            ...
            if return_closest:
                assert out_closest is not None
                out_closest[cs + rep_q[ties]] = closest[ties]
            if return_tri_idx:
                assert out_tri is not None
...
) -> tuple[torch.Tensor, _CentroidTree]:
...
def _dual_contour(
    ...
    scale: float,
    center: torch.Tensor,
    tri_face_normals: torch.Tensor | None = None,
    qef_query=None,
    corner_valid: torch.Tensor | None = None,
) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    R1 = resolution + 1
    keys_per_voxel = (
        corner_pos_per_voxel[..., 0] * R1 + corner_pos_per_voxel[..., 1]
    ) * R1 + corner_pos_per_voxel[..., 2]
    ...
    found = (idx_per_voxel < corner_keys.numel()) & (
        corner_keys[idx_clamped] == keys_per_voxel.reshape(-1)
    )
    ...
    if corner_valid is not None:
        cv_per_voxel = torch.where(
            found, corner_valid[idx_clamped], torch.zeros_like(found)
        ).reshape(Nv, 8)
    ...
    corner_world = (corner_pos_per_voxel.float() / resolution - 0.5) * scale + center.unsqueeze(
        0
    ).unsqueeze(0)  # (Nv, 8, 3)
    ...
    # QEF placement: minimise sum_i (n_i*(x-p_i))^2 via Tikhonov-regularised
    ...
    vox_key = (voxel_coords[:, 0] * vox_dims[1] + voxel_coords[:, 1]) * vox_dims[2] + voxel_coords[
        :, 2
    ]
...
def _dual_contour_manifold(
    ...
    sd = torch.where(found, corner_udf[idx_c], torch.full_like(corner_udf[idx_c], 1.0)).reshape(
        Nv, 8
    )
    ...
    if total_verts == 0:
        return (
            torch.empty((0, 3), dtype=voxel_coords.dtype, device=device),
    ...
    if corner_valid is not None:
        cv = torch.where(found, corner_valid[idx_c], torch.zeros_like(found)).reshape(Nv, 8)
    ...
    if corner_valid is not None:
        edge_group_per_voxel = torch.where(
            crosses, edge_group_per_voxel, torch.full_like(edge_group_per_voxel, -1)
        )
    ...
    t = t.clamp(0.0, 1.0).unsqueeze(-1)
    corner_world = (corner_pos.float() / resolution - 0.5) * scale + center.unsqueeze(0).unsqueeze(
        0
    )
...
def _filter_components(
    ...
    for _ in range(200):
        f_min = torch.minimum(
            torch.minimum(label[faces[:, 0]], label[faces[:, 1]]), label[faces[:, 2]]
        )
    ...
    if drop_enclosed and C > 1:
        ...
        comp_min.scatter_reduce_(
            0, inv[:, None].expand(-1, 3), face_min, reduce="amin", include_self=True
        )
        comp_max.scatter_reduce_(
            0, inv[:, None].expand(-1, 3), face_max, reduce="amax", include_self=True
        )
        big_min = comp_min[large]
        big_max = comp_max[large]
        enclosed = (comp_min >= big_min).all(dim=-1) & (comp_max <= big_max).all(dim=-1)
        ...
        candidates = torch.nonzero(
            (keep & ~enclosed) & (torch.arange(C, device=device) != large), as_tuple=True
        )[0]
...
def _fix_poles(
    verts: torch.Tensor, faces: torch.Tensor, colors: torch.Tensor | None = None
...
def remesh_narrow_band_dc(
    ...
    voxel_coords, _band_tree = _build_narrow_band_voxels(
        vertices, faces, center, scale, resolution, eps, progress_callback=tick
    )
    if voxel_coords.numel() == 0:
        return (
            torch.empty((0, 3), dtype=vertices.dtype, device=device),
            torch.empty((0, 3), dtype=faces.dtype, device=device),
            None
            if colors is None
            else torch.empty((0, colors.shape[1]), dtype=colors.dtype, device=device),
        )
    ...
    # so it works in UDF mode too -- (n*(x-p))^2 is unchanged by normal orientation)
    if use_sdf or qef:
        tri_face_normals_all = torch.nn.functional.normalize(
            torch.cross(
                tri_verts_g[:, 1] - tri_verts_g[:, 0], tri_verts_g[:, 2] - tri_verts_g[:, 0], dim=-1
            ),
            p=2,
            dim=-1,
            eps=1e-12,
    ...
    if qef:
        ...
        def query_qef(pts: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
            return _udf_exact(pts, tri_verts_g, tree=_band_tree)

        _qef_query: (
            Callable[[torch.Tensor], tuple[torch.Tensor, torch.Tensor, torch.Tensor]] | None
        ) = query_qef
    else:
    ...
    if manifold and use_sdf:
        ...
        dual_verts, new_faces = _dual_contour(
            ...
            scale,
            center,
            tri_face_normals=tri_face_normals,
            qef_query=_qef_query,
    ...
    if need_query:
        ...
        if colors is not None:
            _, closest_pts, closest_tri = _udf_query(
                dual_verts,
                tri_verts_g,
                short_hash,
                short_hash_cell_t,
        ...
        if colors is not None:
            assert isinstance(closest_tri, torch.Tensor)
    ...
    if new_faces.numel() > 0 and (
        drop_small_components > 0 or drop_inverted_components or drop_enclosed_components
    ):
    ...
    if fix_poles and new_faces.numel() > 0:
        dual_verts, new_faces, out_colors = _fix_poles(dual_verts, new_faces, out_colors)
    tick()  # post-process done

    if smooth_iters > 0 and dual_verts.numel() > 0 and new_faces.numel() > 0:
        dual_verts = _taubin_smooth(
            dual_verts,
            new_faces,
