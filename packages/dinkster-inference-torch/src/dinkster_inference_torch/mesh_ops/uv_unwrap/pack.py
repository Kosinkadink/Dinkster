...
# Numba is optional, but about 5x faster than Torch for these operations.
try:
    from numba import get_num_threads as _nb_threads
...
class ChartPlacement:
    ...
    swap_xy: bool = False  # extra 90-degree bitmap rotation chosen at place time
...
@_njit(cache=True, boundscheck=False, parallel=True)
def _raster_all_jit(rot_uv, uv_off, faces, f_off, bw, bh, boff, buf, padding, tw, th_out, perim):
    ...
    for fs in range(0, total_faces, _TORCH_RASTER_FACE_BATCH):
        ...
        for g in torch.unique(bsz).tolist():  # one batch per pow2 grid
            ...
            for cs in range(0, sel_g.shape[0], per):
                ...
                vv = (
                    torch.addcmul(
                        d11[sel].view(m, 1, 1) * d20, d01[sel].view(m, 1, 1), d21, value=-1
                    )
                    * idn
                )
                ww = (
                    torch.addcmul(
                        d00[sel].view(m, 1, 1) * d21, d01[sel].view(m, 1, 1), d20, value=-1
                    )
                    * idn
                )
                uu = 1.0 - vv - ww
...
def _build_candidates_gpu(sky_t, ar, cur_w, cur_h, bw0, bw1, step, rand01, device):
    ...
    common = torch.cat(
        [
            torch.stack([xs, zx], 1),
            torch.stack([xs, zx + cur_h], 1),
            torch.stack([zy, ys], 1),
            torch.stack([zy + cur_w, ys], 1),
        ]
    )
    wm = []
    for cw in (bw0, bw1):
        ...
        wm.append(
            max_pool1d(sky_t[:span].view(1, 1, -1).float(), kernel_size=cw, stride=step).view(-1)
        )
    sky = torch.stack([torch.stack([xs, wm[0].long()], 1), torch.stack([xs, wm[1].long()], 1)])
...
def _pack_bitmap_torch(
    ...
    while start < n:
        ...
        uvs_batch = torch.stack(
            [(rx - rxmin[:, None]) * scale[:, None], (ry - rymin[:, None]) * scale[:, None]], dim=-1
        )
    ...
    cur_w = cur_h = 0
    placements: list[ChartPlacement | None] = [None] * n
    ...
    for t_i, ci in enumerate(order):
        ...
        res = _best_placement_torch(atlas, pix_l[ci], dim, dimr, cands, n_sky, cur_w, cur_h, device)
    ...
    return cast(list[ChartPlacement], placements), cur_w, cur_h
...
def pack_bitmap_concat(
    ...
    if not _HAVE_NUMBA_PACK:
        placements, w, h = _pack_bitmap_torch(
            uvs_cat,
            uv_offsets,
            [float(a) for a in chart_3d_areas],
            [float(a) for a in chart_uv_areas],
            faces_cat,
            face_offsets,
            texels_per_unit,
            padding_texels,
    ...
    _raster_all_jit(
        rot_uv, uv_off, faces64, f_off, bw, bh, boff, buf, int(padding_texels), tw, th_arr, perim
    )
