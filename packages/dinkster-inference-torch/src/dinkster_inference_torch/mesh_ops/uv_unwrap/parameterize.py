...
def solve_least_squares(A: sp.csr_matrix, b: np.ndarray) -> np.ndarray:
    ...
    AtA = (At @ A).tocsc()
    Atb = At @ b
    return np.asarray(spla.spsolve(AtA, Atb))
...
def ortho_project_concat(verts: np.ndarray, chart_of_vert: np.ndarray, n_charts: int) -> np.ndarray:
    ...
    cen = (
        np.stack(
            [np.bincount(chart_of_vert, weights=verts[:, i], minlength=n_charts) for i in range(3)],
            axis=1,
        )
        / cnt[:, None]
    )
...
def stretch_metrics_concat(
    ...
    inv_2pa = 1.0 / (2.0 * pa)
    Ss = (
        p[:, 0] * (t2 - t3)[:, None] + p[:, 1] * (t3 - t1)[:, None] + p[:, 2] * (t1 - t2)[:, None]
    ) * inv_2pa[:, None]
    St = (
        p[:, 0] * (s3 - s2)[:, None] + p[:, 1] * (s1 - s3)[:, None] + p[:, 2] * (s2 - s1)[:, None]
    ) * inv_2pa[:, None]
    a = (Ss * Ss).sum(axis=1)
    bb = (Ss * St).sum(axis=1)
    c = (St * St).sum(axis=1)
...
def lscm_charts_batch(
    ...
    for s, e in chunks:
        ...
        A = np.bincount(flat, weights=np.concatenate(pieces_v), minlength=B * R * N).reshape(
            B, R, N
        )
        ...
        pin_vals = np.stack(
            [
                uv_pins[pin_a[cids], 0],
                uv_pins[pin_b[cids], 0],
                uv_pins[pin_a[cids], 1],
                uv_pins[pin_b[cids], 1],
            ],
            1,
        )
...
def _uv_boundary_self_intersects(
    ...
    for s in range(0, n, chunk):
        ...
        cross = (np.abs(denom) >= eps) & (t > eps) & (t < 1.0 - eps) & (u > eps) & (u < 1.0 - eps)
        if bool(cross.any()):
            return True
    return False
...
def lscm_chart(
    ...
    if invalid_idx.size:
        ...
        twice_area_inv = (
            tri2d_inv[:, 1, 0] * tri2d_inv[:, 2, 1] - tri2d_inv[:, 1, 1] * tri2d_inv[:, 2, 0]
        )
