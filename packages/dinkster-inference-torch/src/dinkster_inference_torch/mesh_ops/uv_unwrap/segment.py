...
NORMAL_DEVIATION_HARD_CUTOFF = 0.707  # about 75 degrees
...
def _grow_iter(
    ...
    round_cost = torch.where(
        (cp <= 1e-20) | (ca <= 1e-20) | (new_r <= 1e-20),
    ...
    dper = torch.where(
        nbc_old == cx,
        -el_acc,  # was member: edge turns interior
        torch.where(
            nbc_post == cx,
            torch.zeros_like(el_acc),  # co-committer
            el_acc,
        ),
    ).sum(1)  # boundary / other chart
...
def segment_charts(
    ...
    while True:
        ...
        new_seed = int(torch.where(unassigned, min_d2, min_d2.new_full((), float("-inf"))).argmax())
...
def _combine_normal_cones(
    ...
    new_axis = axis_a * torch.cos(rot_angle).unsqueeze(-1) + b_perp_unit * torch.sin(
        rot_angle
    ).unsqueeze(-1)
...
def _build_chart_edges(
    ...
    unique_key, inverse, counts = torch.unique(sorted_key, return_inverse=True, return_counts=True)
...
def _merge_small_charts(
    ...
    for _ in range(16):
        ...
        E = edges.shape[0]
        key = (torch.clamp(new_half * 1e6, max=2e9).to(torch.int64) << 32) | torch.arange(
            E, dtype=torch.long, device=device
        )
...
def cluster_charts_pec(
    ...
    if min_faces > 1:
        chart_id = _merge_small_charts(
            chart_id,
            face_normal,
            mesh.face_area.to(torch.float32),
            face_face,
            face_edge_len,
            min_faces,
            2.0 * max_cost,
