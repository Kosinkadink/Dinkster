...
class QEMConfig:
    ...
    wander_threshold: float = (
        2.0  # fall back to midpoint if v* lands > N*edge_length from an endpoint
    )
    ...
    flip_reject_hard: bool = (
        True  # hard-reject (err=+inf) top-K collapses that flip any 1-ring normal
    )
    ...
    max_collapses_relative_cap: float = (
        0.10  # cap per-iter collapses as fraction of current faces; 0 disables
    )
    ...
    line_quadric_weight: float = (
        0.0  # penalise deviation perpendicular to edge dir -> more uniform verts; 0 disables
    )
    line_quadric_skip_opposite_normals_cos: float = (
        0.0  # skip line quadrics on edges with endpoint cos < this
    )
    ...
    # Flip check (FA-QEM section 3.3)
    quality_topk_multiplier: int = 4  # quality-check band size = this * max_collapses_per_iter
    flip_cos_threshold: float = 0.0  # 0 = count any sign reversal (dihedral > 90 degrees)
    ...
    lambda_edge_length: float = 1e-2  # add lambda*len^2 to bias toward short edges; 0 disables
    lambda_edge_length_absolute: bool = True  # apply lambda absolutely vs relative-to-QEM-median
...
def _manifold_edge_pairs(
    ...
    return (sorted_keys[pair_starts], sorted_faces[pair_starts], sorted_faces[pair_starts + 1])


def _line_quadric_planes(
    pa: torch.Tensor, pb: torch.Tensor
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """Two plane equations (E,4) per edge whose squared-dist sum is squared perpendicular distance."""
    e = pb - pa  # (E, 3)
...
def _add_line_quadrics(
    ...
    Q_flat: torch.Tensor,
    weight: float,
    skip_he_mask: torch.Tensor | None = None,
) -> torch.Tensor:
    ...
    K_line = (
        p_u.unsqueeze(-1) * p_u.unsqueeze(-2) + p_w.unsqueeze(-1) * p_w.unsqueeze(-2)
    ) * w_per_edge
...
def _build_quadrics(
    ...
    if faces.numel() > 0:
        ...
        n_norm = torch.where(
            mask.unsqueeze(-1), n / area.unsqueeze(-1).clamp_min(1e-12), n.new_zeros(())
        )
    ...
    # Line quadrics: squared perpendicular distance from v to the edge-midpoint line in one pass.
    if cfg.line_quadric_weight > 0 and faces.numel() > 0:
        ...
        if cfg.line_quadric_skip_opposite_normals_cos < 1.0:
            ...
            n_weighted = n_norm * area.unsqueeze(-1)  # normal * twice the area
            for corner in range(3):
                v_norm.scatter_add_(0, faces[:, corner].unsqueeze(-1).expand(-1, 3), n_weighted)
            ...
            if not skip_he_sharp.any():
                skip_he_sharp = None
        Q_flat = _add_line_quadrics(
            verts, faces, area, Q_flat, cfg.line_quadric_weight, skip_he_mask=skip_he_sharp
        )
    ...
    if cfg.boundary_quadrics and faces.numel() > 0:
        ...
        if b_edges.shape[0] > 0:
            ...
            p_u, p_w, _ = _line_quadric_planes(pa, pb)
            K_b = (
                torch.einsum("ei,ej->eij", p_u, p_u) + torch.einsum("ei,ej->eij", p_w, p_w)
            ) * cfg.boundary_weight
    ...
    if cfg.feature_edge_quadric_weight > 0 and faces.numel() > 0:
        ...
        if pair_keys.numel() > 0:
            ...
            if sharp.any():
                ...
                w = (
                    (avg_area * sharpness * cfg.feature_edge_quadric_weight)
                    .unsqueeze(-1)
                    .unsqueeze(-1)
                )
                K_feat = (
...
def _edge_errors(
    ...
    Q: torch.Tensor,
    edges: torch.Tensor,
    stabilizer: float | torch.Tensor,
    max_edge_length_sq: float | torch.Tensor,
    mesh_scale_sq: float | torch.Tensor,
    cfg: QEMConfig,
    vert_is_boundary: torch.Tensor | None = None,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    if vert_is_boundary is not None:
        ...
        w_a = torch.where(
            ba & ~bb,
            torch.ones_like(el),
            torch.where(~ba & bb, torch.zeros_like(el), torch.full_like(el, 0.5)),
        )
    ...
    if cfg.placement_mode == "midpoint":
        ...
        if cfg.clamp_v_to_edge:
            ...
            wander_bad = (
                (dist_a > cfg.wander_threshold * el) | (dist_b > cfg.wander_threshold * el)
            ).unsqueeze(-1)
...
def _normal_flip_mask(
    ...
    for start in range(0, E, chunk_size):
        ...
        safe = denom > 1e-20
        cos = torch.where(
            safe, (n_old * n_new).sum(dim=-1) / denom.clamp_min(1e-20), torch.ones_like(denom)
        )
...
def _link_condition_mask(
    ...
    """(E,) mask -- True where collapse is topology-safe (common neighbours <= edge faces)."""
    E = edges.shape[0]
    device = faces.device
    out = torch.ones(E, dtype=torch.bool, device=device)
    if E == 0:
    ...
    for start in range(0, E, chunk_size):
        ...
        if need_geom:
            ...
            if affected.any():
                ...
                # post-collapse normal (skinny's two_area == flip's norm(n_new))
                e01 = p1n - p0n
                e02 = p2n - p0n
                ...
                if want_flip:
                    ...
                    cos = torch.where(
                        safe,
                        (n_old * n_new).sum(dim=-1) / denom.clamp_min(1e-20),
...
def _compute_vertex_normals(
    verts: torch.Tensor, faces: torch.Tensor, weld: bool = True
) -> torch.Tensor:
...
class CleanStats:
    ...
    out_verts: int = 0
    out_faces: int = 0
    welded_verts: int | torch.Tensor = 0  # how many vertex IDs collapsed during welding
    degenerate_faces: int | torch.Tensor = 0  # zero-area or repeated-index faces removed
    duplicate_faces: int | torch.Tensor = 0  # same vertex-set removed
    unused_verts: int | torch.Tensor = 0  # verts not in any face removed
    components_dropped: int | torch.Tensor = 0  # disconnected components below threshold

    def __str__(self):
        return (
            f"clean: in={self.in_verts}v/{self.in_faces}f -> "
...
def _collapse_slivers(
    ...
    shortest_slot = edge_lens.argmin(dim=-1)  # (F,) in {0,1,2}
...
def _drop_duplicate_faces(faces: torch.Tensor, num_verts: int) -> tuple[torch.Tensor, int]:
    ...
    first = torch.full(
        (unique_packed.shape[0],), packed.shape[0], dtype=torch.int64, device=packed.device
    )
...
def _repair_nonmanifold_edges(
    ...
    return (
        torch.from_numpy(new_verts).to(device=dev, dtype=vdt),
...
def clean_mesh(
    ...
    min_angle_deg: float = 0.0,
    max_aspect_ratio: float = 0.0,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor | None, torch.Tensor | None, CleanStats]:
...
def qem_simplify(
    ...
    config: QEMConfig | None = None,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor | None, torch.Tensor | None, SimplifyStats]:
    ...
    normals_w = (
        normals.to(device=device, dtype=cfg.dtype, copy=True) if normals is not None else None
    )
    ...
    if max_edge_length is None or max_edge_length <= 0:
        max_edge_length_tensor = mesh_scale * 2.0
    else:
        max_edge_length_tensor = torch.as_tensor(max_edge_length, dtype=cfg.dtype, device=device)
    # tiny-bbox guard (tensor-side, no sync)
    max_edge_length_tensor = torch.where(
        max_edge_length_tensor < 1e-6,
        torch.ones((), dtype=max_edge_length_tensor.dtype, device=device),
        max_edge_length_tensor,
    )

    stabilizer = mesh_scale * mesh_scale * cfg.stabilizer_scale
    max_edge_length_sq = max_edge_length_tensor * max_edge_length_tensor
    mesh_scale_sq = mesh_scale * mesh_scale

    # threshold scaled by mesh_scale^2 so the 1e-8 start is scale-robust
    ...
    while True:
        ...
        edges_orig = edges_orig[el < max_edge_length_tensor]
        ...
        if cfg.threshold_driver:
            ...
            if (need_flip or cfg.skinny_weight > 0 or cfg.enforce_link_condition) and ce.shape[
                0
            ] > 0:
                afq = faces[alive_f]
                ...
                fc, sk, link_safe = _quality_checks_fused(
                    ...
                    copt,
                    v_to_f,
                    cos_threshold=cfg.flip_cos_threshold,
                    want_flip=need_flip,
                    want_skinny=(cfg.skinny_weight > 0),
            ...
            need_flip = cfg.flip_reject_hard
            need_quality = (
                need_flip or cfg.skinny_weight > 0 or cfg.enforce_link_condition
            ) and edges_orig.shape[0] > 0
            if need_quality:
                n_check = min(
                    edges_orig.shape[0], max(1, cfg.quality_topk_multiplier * max_collapses)
                )
                ...
                v_to_f = _build_vert_to_faces_pad(
                ...
                if _do_flip and _do_skinny:
                    ...
                    flip_count = None
                if _do_flip:
                    assert flip_count is not None
                    # hard reject: any flipping top-K edge -> +inf
                    flips = flip_count > 0
                    if flips.any():
                        err[topk] = torch.where(
                            flips,
                            torch.full_like(err[topk], float("inf")),
                            err[topk],
                        )
                if _do_skinny:
                    assert skinny is not None
                    # skinny_cost * len^2 (match QEM's length^2 scaling)
                    elen_sq = (verts[e_tk[:, 1]] - verts[e_tk[:, 0]]).pow(2).sum(dim=-1)
                    err[topk] = torch.add(err[topk], skinny * elen_sq, alpha=cfg.skinny_weight)
    ...
    if cfg.postclean and final_f.numel() > 0:
        ...
        final_v, final_f, final_c, final_n, _ps = clean_mesh(
            ...
            final_n,
            weld_epsilon=0.0,
            weld_epsilon_rel=0.0,  # already welded
    ...
    if cfg.recompute_normals_post and final_f.numel() > 0:
        ...
        fn = torch.cross(v1 - v0, v2 - v0, dim=-1)
        ref = (final_n[final_f[:, 0]] + final_n[final_f[:, 1]] + final_n[final_f[:, 2]]) / 3.0
    ...
    if vertices.ndim == 3:
        ...
        for i in range(vertices.shape[0]):
            ...
            v, f, c, n, s = qem_simplify(
                vertices[i], faces[i], target, c_in, n_in, max_edge_length, config
            )
            out_v.append(v)
            out_f.append(f)
            out_s.append(s)
            ...
            if n is not None:
                out_n.append(n)
        return (out_v, out_f, out_c if out_c else None, out_n if out_n else None, out_s)
...
def qem_cluster_decimate(
    ...
    # cell size so the bbox holds about 3*target_verts cells (surface occupancy about 1/3)
    ...
    if new_faces.numel() > 0:
        ...
        P = n_unique + 1
        packed = (key_sorted[:, 0].long() * P + key_sorted[:, 1].long()) * P + key_sorted[
            :, 2
        ].long()
        ...
        first_idx = torch.full(
            (int(first.max().item()) + 1,), packed.shape[0], dtype=torch.int64, device=device
        )
