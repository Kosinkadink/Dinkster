...
def wavelet_decomposition(
    image: Tensor, levels: int = WAVELET_DECOMP_LEVELS
) -> tuple[Tensor, Tensor]:
...
def wavelet_reconstruction(content_feat: Tensor, style_feat: Tensor) -> Tensor:
    ...
    if content_high_freq.shape != style_low_freq.shape:
        style_low_freq = F.interpolate(
            style_low_freq, size=content_high_freq.shape[-2:], mode="bilinear", align_corners=False
        )
...
def lab_color_transfer(
    content_feat: Tensor, style_feat: Tensor, luminance_weight: float = 0.8
) -> Tensor:
    ...
    if luminance_weight < 1.0:
        ...
        result_L = (
            content_lab[:, 0].mul(luminance_weight).add_(matched_L.mul(1.0 - luminance_weight))
        )
        del matched_L
    else:
