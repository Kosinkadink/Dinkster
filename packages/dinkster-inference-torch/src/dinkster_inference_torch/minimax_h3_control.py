...
from .minimax_h3_dit import (
    ...
    _MLP_TIME_EMBED_DIM,  # pyright: ignore[reportPrivateUsage]
...
def minimax_h3_fun_injection_layers(
    ...
    if control_blocks_places is None:
        if num_blocks > _BASE_BLOCK_COUNT or _BASE_BLOCK_COUNT % num_blocks:
            raise ValueError(
                "MiniMax H3 Fun control blocks must divide the fifty base blocks evenly"
            )
        injection_layers = tuple(range(0, _BASE_BLOCK_COUNT, _BASE_BLOCK_COUNT // num_blocks))
    else:
    ...
    if (
        not injection_layers
        or injection_layers[0] != 0
        or injection_layers != tuple(sorted(set(injection_layers)))
        or any(
            type(index) is not int or not 0 <= index < _BASE_BLOCK_COUNT
            for index in injection_layers
        )
    ):
        raise ValueError(
            "MiniMax H3 Fun control injection layers must be unique increasing "
            "base-block indices starting at zero"
        )
...
def prepare_minimax_h3_fun_control_hint(
    ...
    *,
    ...
    """Prepare the 24- or 49-channel latent consumed by H3 Fun control."""
    if len(target_shape) != 5 or target_shape[:2] != (1, 24):
    ...
    def fit(frames: torch.Tensor) -> torch.Tensor:
        if frames.ndim != 4 or frames.shape[0] < 1:
            raise ValueError(
                "MiniMax H3 control frames must be non-empty [frames,channels,height,width]"
            )
        indices = torch.arange(frame_count, device=frames.device).clamp(max=frames.shape[0] - 1)
        return common_upscale(frames[indices], width, height, "bilinear", "center")

    def encode_fitted(frames: torch.Tensor) -> torch.Tensor:
        latent = encode(frames)
    ...
    hint = None if control_video is None else encode_fitted(fit(control_video))
    if mask is None:
        if hint is None:
            raise ValueError("MiniMax H3 Fun control requires control video or mask input")
        return hint

    control_mask = mask.reshape(-1, 1, *mask.shape[-2:])
    control_mask = (control_mask > 0.5).to(torch.float32)
    indices = torch.arange(frame_count, device=control_mask.device).clamp(
        max=control_mask.shape[0] - 1
    )
    control_mask = common_upscale(control_mask[indices], width, height, "bilinear", "center")
    visibility = 1.0 - (control_mask > 0.5).to(torch.float32)
    source = (
        torch.zeros(
            frame_count,
            3,
        ...
        else fit(source_video)
    )
    masked = minimax_h3_inpaint_mask_fill(source, visibility, inpaint_post_norm)
    masked_latent = encode_fitted(masked)
    if hint is None:
    ...
    return torch.cat([hint, visibility_latent, masked_latent.to(hint.device)], dim=1)
