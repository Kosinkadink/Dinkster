...
Self-contained: the reference decode below restates the Q4_0 layout
(float16 scale at bytes 0:2, 16 packed quant bytes at bytes 2:18;
elements 0..15 are the low nibbles and 16..31 the high nibbles,
decode = scale * (nibble - 8)) rather than importing
...
requires_cuda = pytest.mark.skipif(
    ...
    reason="fused Q4_0 kernels need a CUDA device and triton",
)
...
def _reference_decode(blocks: torch.Tensor, out_features: int, in_features: int) -> torch.Tensor:
    ...
    packed = blocks[:, 2:]
    nibbles = torch.cat([packed & 0x0F, packed >> 4], dim=1).to(torch.float32) - 8.0
    return (nibbles * scales).reshape(out_features, in_features)
...
def _random_blocks(
    ...
    count = out_features * in_features // Q4_0_BLOCK_ELEMENTS
    blocks = torch.randint(
        0, 256, (count, Q4_0_BLOCK_BYTES), dtype=torch.uint8, generator=generator
    )
...
def _skip_unless_available() -> None:
    if not gguf_q4_0_linear_available():
        pytest.skip("fused Q4_0 probe reports unavailable (no host C compiler for triton?)")
...
def test_import_and_probe_never_raise() -> None:
    ...
    first = gguf_q4_0_linear_available()
    assert isinstance(first, bool)
    assert gguf_q4_0_linear_available() == first


def test_block_constants() -> None:
    assert Q4_0_BLOCK_ELEMENTS == 32
    assert Q4_0_BLOCK_BYTES == 18
...
def test_decode_bit_exact_vs_reference(out_features: int, in_features: int) -> None:
    ...
    # A negative scale over centered quants (nibble 8 -> 0) decodes to
    # -0.0: only a bit-pattern compare can tell it from +0.0, so pin
    # one such block.
    blocks[0, 2:] = 0x88
    ...
    decoded = gguf_q4_0_decode(blocks.cuda(), out_features, in_features)
...
def test_linear_matches_decode_reference(dtype: torch.dtype, m: int, n: int, k: int) -> None:
    ...
    weight = gguf_q4_0_decode(blocks, n, k).to(dtype)
    for b in (None, bias):
        fused = gguf_q4_0_linear(x, blocks, b, n)
...
def test_linear_batched_input_shape() -> None:
    ...
    out = gguf_q4_0_linear(x, blocks, None, 64)
    assert out.shape == (2, 5, 64)
    flat = gguf_q4_0_linear(x.reshape(-1, 32), blocks, None, 64)
...
def test_linear_deterministic() -> None:
    ...
    first = gguf_q4_0_linear(x, blocks, None, 96)
    second = gguf_q4_0_linear(x, blocks, None, 96)
...
def test_validation_errors() -> None:
    ...
    with pytest.raises(ValueError, match="uint8"):
        gguf_q4_0_decode(good.to(torch.int8), 64, 32)
    with pytest.raises(ValueError, match="whole 32-element blocks"):
        gguf_q4_0_linear(torch.randn((4, 48), dtype=torch.float16, device="cuda"), good, None, 64)
    with pytest.raises(ValueError, match="do not hold"):
        gguf_q4_0_decode(good, 64, 64)
    with pytest.raises(ValueError, match="float16 or bfloat16"):
        gguf_q4_0_linear(x.to(torch.float32), good, None, 64)
    with pytest.raises(ValueError, match="bias"):
        gguf_q4_0_linear(x, good, torch.randn(64, dtype=torch.float32, device="cuda"), 64)
    with pytest.raises(ValueError, match="blocks on the input device"):
        gguf_q4_0_linear(x, good.cpu(), None, 64)
    with pytest.raises(ValueError, match="bias on the input device"):
        gguf_q4_0_linear(x, good, torch.randn(64, dtype=torch.float16), 64)
...
def test_linear_noncontiguous_bias() -> None:
    ...
    out = gguf_q4_0_linear(x, blocks, strided, 64)
    assert torch.equal(out, gguf_q4_0_linear(x, blocks, strided.contiguous(), 64))
...
def test_opcheck_both_ops() -> None:
    ...
    torch.library.opcheck(gguf_q4_0_decode, (blocks, 64, 32))
    torch.library.opcheck(gguf_q4_0_linear, (x, blocks, bias, 64))
    torch.library.opcheck(gguf_q4_0_linear, (x, blocks, None, 64))
