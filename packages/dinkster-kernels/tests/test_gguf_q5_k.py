...
Self-contained: the reference decode below restates the Q5_K layout
(the Q4_K scale layout - float16 d at bytes 0:2, dmin at 2:4, twelve
packed 6-bit (scale, min) bytes at 4:16 - then a 32-byte high-bit
plane at 16:48 and 128 low-nibble bytes at 48:176 as four 32-byte
chunks; element e's fifth quant bit is bit 2c + h of plane byte
e % 32, and each element decodes as q * (d * sc) - (dmin * mn))
rather than importing dinkster-inference-torch, so this package tests
its own numerical contract in isolation. Cross-package bit-identity
against the inference decoders lives in dinkster-inference-torch's GPU
suite.
"""
...
requires_cuda = pytest.mark.skipif(
    ...
    reason="fused Q5_K kernels need a CUDA device and triton",
)
...
def _reference_decode(blocks: torch.Tensor, out_features: int, in_features: int) -> torch.Tensor:
    ...
    plane = blocks[:, 16:48].reshape(-1, 1, 1, 32)
    low = blocks[:, 48:].reshape(-1, 4, 1, 32)
    shifts = torch.arange(8, dtype=torch.uint8).reshape(1, 4, 2, 1)
    bits = (plane >> shifts) & 1
...
def _random_blocks(
    ...
    count = out_features * in_features // Q5_K_BLOCK_ELEMENTS
    blocks = torch.randint(
        0, 256, (count, Q5_K_BLOCK_BYTES), dtype=torch.uint8, generator=generator
    )
...
def _pin_negative_zero_block(blocks: torch.Tensor) -> None:
    """Make block 0 decode to all -0.0: zero quants (nibbles and high
    bits) and mins under a negative group scale, so q * (d * sc) is
    -0.0 and the min subtraction preserves the sign. Only a
    bit-pattern compare can tell the result from +0.0."""
    blocks[0, 4:] = 0
...
def _skip_unless_available() -> None:
    if not gguf_q5_k_linear_available():
        pytest.skip("fused Q5_K probe reports unavailable (no host C compiler for triton?)")
...
def test_import_and_probe_never_raise() -> None:
    ...
    first = gguf_q5_k_linear_available()
    assert isinstance(first, bool)
    assert gguf_q5_k_linear_available() == first


def test_block_constants() -> None:
    assert Q5_K_BLOCK_ELEMENTS == 256
    assert Q5_K_BLOCK_BYTES == 176
...
def test_decode_bit_exact_vs_reference(out_features: int, in_features: int) -> None:
    ...
    decoded = gguf_q5_k_decode(blocks.cuda(), out_features, in_features)
...
def test_linear_matches_decode_reference(dtype: torch.dtype, m: int, n: int, k: int) -> None:
    ...
    weight = gguf_q5_k_decode(blocks, n, k).to(dtype)
    for b in (None, bias):
        fused = gguf_q5_k_linear(x, blocks, b, n)
...
def test_linear_batched_input_shape() -> None:
    ...
    out = gguf_q5_k_linear(x, blocks, None, 64)
    assert out.shape == (2, 5, 64)
    flat = gguf_q5_k_linear(x.reshape(-1, 256), blocks, None, 64)
...
def test_linear_deterministic() -> None:
    ...
    first = gguf_q5_k_linear(x, blocks, None, 96)
    second = gguf_q5_k_linear(x, blocks, None, 96)
...
def test_validation_errors() -> None:
    ...
    with pytest.raises(ValueError, match="uint8"):
        gguf_q5_k_decode(good.to(torch.int8), 64, 256)
    with pytest.raises(ValueError, match="whole 256-element blocks"):
        gguf_q5_k_linear(torch.randn((4, 384), dtype=torch.float16, device="cuda"), good, None, 64)
    with pytest.raises(ValueError, match="do not hold"):
        gguf_q5_k_decode(good, 64, 512)
    with pytest.raises(ValueError, match="float16 or bfloat16"):
        gguf_q5_k_linear(x.to(torch.float32), good, None, 64)
    with pytest.raises(ValueError, match="bias"):
        gguf_q5_k_linear(x, good, torch.randn(64, dtype=torch.float32, device="cuda"), 64)
    with pytest.raises(ValueError, match="blocks on the input device"):
        gguf_q5_k_linear(x, good.cpu(), None, 64)
    with pytest.raises(ValueError, match="bias on the input device"):
        gguf_q5_k_linear(x, good, torch.randn(64, dtype=torch.float16), 64)
...
def test_linear_noncontiguous_bias() -> None:
    ...
    out = gguf_q5_k_linear(x, blocks, strided, 64)
    assert torch.equal(out, gguf_q5_k_linear(x, blocks, strided.contiguous(), 64))
...
def test_opcheck_both_ops() -> None:
    ...
    torch.library.opcheck(gguf_q5_k_decode, (blocks, 64, 256))
    torch.library.opcheck(gguf_q5_k_linear, (x, blocks, bias, 64))
    torch.library.opcheck(gguf_q5_k_linear, (x, blocks, None, 64))
