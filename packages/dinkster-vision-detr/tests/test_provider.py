...
from dinkster_vision_detr import register_types
from dinkster_vision_detr.model import COCO_CLASSES, execute_detect, load_model, prepare_frame
from dinkster_workers import IsolatedWorker, load_manifest

ROOT = Path(__file__).parents[3]
MANIFEST = ROOT / "packages" / "dinkster-vision-detr" / "dinkster-pack.toml"
...
def test_detr_outputs_match_pinned_reference_vectors(tmp_path: Path) -> None:
    ...
    np.testing.assert_array_equal(boxes.numpy(), _decode_float32(golden["boxes"]))
...
def test_invalid_max_results_fails_before_loading_model(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("dinkster_vision_detr.model.load_model", unexpected_load)
