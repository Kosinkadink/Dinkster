"""Generate exact DPM++ SDE float32 trajectories from pinned ComfyUI.
...
Usage from the Dinkster repository root:
    ...
    PYTHONPATH=/path/to/ComfyUI-at-b78cec87 \
        .venv-gpu/bin/python tools/gen_dpmpp_sde_trajectory_goldens.py
"""
...
OUT = platform_golden_path(
    REPO / "packages/dinkster-inference-torch/tests/goldens/dpmpp_sde_trajectory_goldens.json",
    torch.__version__,
)
...
NOISE_DRAWS = (
    ...
    (0.854598908, 0.501020856, -0.174326781, -0.76768581),
    (0.222889914, -0.442520443, -0.916165937, -0.919328526),
    (-0.536572918, -0.943401482, -0.875452175, -0.457535894),
)
...
def _case(space: object, sigmas: tuple[float, ...]) -> dict[str, object]:
    ...
    result = sampling.sample_dpmpp_sde(
        model,
        initial.clone(),
        schedule,
        extra_args={},
...
def main() -> None:
    ...
    source_path = Path(inspect.getfile(inspect.unwrap(sampling.sample_dpmpp_sde))).resolve()
    try:
        source_relative = source_path.relative_to(COMFY_ROOT)
    except ValueError as error:
        raise SystemExit(
            f"refusing to generate: imported DPM++ SDE from {source_path}, outside {COMFY_ROOT}"
        ) from error

    data = {
        "_meta": {
            "generator": "tools/gen_dpmpp_sde_trajectory_goldens.py",
