"""Persistent ComfyUI adapter for the canonical SDXL EDM v-pred workload."""
...
def main() -> int:
    ...
    import comfy.model_sampling  # pyright: ignore[reportMissingImports]
    ...
    if args.construct_only:
        ...
        with safe_open(checkpoint_path, framework="pt", device="cpu") as checkpoint:
            sigma_min = checkpoint.get_tensor("edm_vpred.sigma_min")
            sigma_max = checkpoint.get_tensor("edm_vpred.sigma_max")
        if (
            sigma_min.shape != torch.Size([])
            or sigma_min.dtype is not torch.float32
            or sigma_min.item() != 0.0020000000949949026
            or sigma_max.shape != torch.Size([])
            or sigma_max.dtype is not torch.float32
            or sigma_max.item() != 120.0
        ):
            raise RuntimeError(f"unexpected EDM v-pred markers: {sigma_min!r}; {sigma_max!r}")
        print(
            "READY comfyui cpu=True; nodes imported; edm_vpred markers pinned; "
    ...
    for line in sys.stdin:
        with torch.inference_mode():
            ...
            if checkpoint is None:
                ...
                if not isinstance(
                    sampling, comfy.model_sampling.ModelSamplingContinuousEDM
                ) or not isinstance(sampling, comfy.model_sampling.V_PREDICTION):
                    raise RuntimeError(f"unexpected model sampling type: {type(sampling).__mro__}")
                if float(sampling.sigma_min) != 0.001999999862164259:
                    raise RuntimeError(
                        f"unexpected effective sigma_min: {float(sampling.sigma_min)!r}"
                    )
                if float(sampling.sigma_max) != 120.00000762939453:
                    raise RuntimeError(
                        f"unexpected effective sigma_max: {float(sampling.sigma_max)!r}"
                    )
