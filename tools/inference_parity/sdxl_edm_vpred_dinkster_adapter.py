"""Persistent native Dinkster adapter for the canonical SDXL EDM v-pred workload."""
...
def main() -> int:
    ...
    if args.construct_only:
        ...
        if (
            ...
            or sampling.space is not SamplingSpace.CONTINUOUS_EDM
            or sampling.sigma_min != 0.0020000000949949026
            or sampling.sigma_max != 120.0
            or sampling.zsnr
        ):
            raise RuntimeError(
                f"SDXL continuous-EDM v-pred checkpoint was not accepted: {capability}; {sampling}"
            )
        print(
